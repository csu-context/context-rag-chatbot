...
def check_dependencies():
    ...
    try:
        import anthropic  # noqa: F401 — 가용성 검사 목적
    except ImportError:
...
class DoclingPDFParser:
    ...
    def __init__(self):
        try:
            ...
            raise ImportError("docling 라이브러리가 필요합니다.\n설치: pip install docling docling-core") from err
...
**응답 형식 (JSON만):**
```json
{{"korean_integrity": <1-5>, "structure": <1-5>, "tables": <1-5>,
"readability": <1-5>, "overall": <1-5>, "notes": "<의견>"}}
"""
        ...
        try:
            ...
            logging.warning(f"Claude 응답 JSON 파싱 실패: {e}")
        except Exception as e:
...
class BenchmarkRunner:
    ...
    def run(self) -> dict:
        ...
        results = {"file": self.pdf_path.name, "timestamp": time.time(), "parsers": {}}

        # 1. PyMuPDF
        self.logger.info("1/3: PyMuPDF 파싱...")
        pymupdf_result: dict | None = None
        try:
        ...
        unstructured_result: dict | None = None
        try:
        ...
        if ref_result and docling_result:
            results["comparison"] = {
                ...
                "pymupdf_vs_docling": {
                    "cer": TextMetrics.calculate_cer(ref_result["text"], docling_result["text"]),
            ...
            if unstructured_result:
                results["comparison"]["pymupdf_vs_unstructured"] = {
                    "cer": TextMetrics.calculate_cer(ref_result["text"], unstructured_result["text"]),
    ...
    def _evaluate(self, parse_result: dict) -> dict:
        ...
        judge = LLMJudge()
        claude_eval = judge.evaluate(parse_result["text"], parse_result["tables"], self.pdf_path.name)

        # overall > 0 이면 저장 (기본값 0은 평가 실패을 의미)
        if claude_eval.get("overall", 0) > 0:
            eval_result["claude"] = claude_eval
        else:
            logging.warning(f"Claude Judge 응답 파싱 실패 - 엔진: {eval_result.get('engine', '?')}")
