...
VLM_PROMPT = (
    ...
    "규칙:\n"
    "1) 모든 행의 학위 칸을 반드시 채워라. 빈칸을 남기지 마라.\n"
    "2) 학위가 세로병합으로 여러 학과(부)를 하나로 묶고 있으면, 묶인 범위의 모든 행"
    "(학부 행과 그 아래 하위 전공 행 전부)에 같은 학위를 반복해서 기입하라.\n"
    "3) 괄호로 들여쓴 (OOO전공)은 바로 위 학부(과)의 하위 전공이며, 그 학위는 자신이 속한 "
    "세로병합 블록의 학위를 그대로 따른다.\n"
    "4) 학과(부)와 학위를 같은 행끼리 정확히 대응시켜라(위/아래 행으로 밀리지 않게).\n"
    "5) 대학명도 세로병합이면 모든 행에 채워라.\n"
    "6) 설명 없이 마크다운 표만 출력하라."
)
