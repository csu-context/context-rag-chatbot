...
from src.utils.paths import VECTOR_DB_DIR, BACKUP_DIR, LOGS_DIR, ensure_directories
...
def restore_chromadb(backup_file=None):
    ...
    if backup_file:
        ...
        # 최신 백업 파일 찾기
        backups = sorted(
            list(BACKUP_DIR.glob("chromadb_backup_*.tar.gz")),
            key=lambda x: x.name,
            reverse=True
        )
        if not backups:
    ...
    try:
        ...
        logger.error("복원 실패: 파일 잠금(Lock)이 발생했습니다. ChromaDB를 사용하는 다른 프로세스를 종료 후 다시 시도하십시오.")
        return False
