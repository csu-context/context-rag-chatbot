...
def show_admin_dialog():
    ...
    with col_opt1:
        auto_sync = st.checkbox("파일 업로드/삭제 후 자동 동기화 실행", value=True, help="체크 시 별도의 Sync 버튼 클릭 없이 즉시 DB에 반영합니다.")
    ...
    if not current_files:
        ...
        h_col4.write("**청크**")
        h_col5.write("**삭제**")
        st.markdown("<hr style='margin: 0px 0px 10px 0px; border: 0.5px solid rgba(151,166,195,0.2);'>", unsafe_allow_html=True)
        ...
        for i, f in enumerate(current_files):
            ...
            if r_col5.button("🗑️", key=f"del_btn_{i}", help=f"'{f.name}' 삭제"):
                if f.exists():
                    f.unlink()
...
class StreamUIHandler:
    ...
    def _handle_generation(self, state, step):
        if state == "running": self.generation_msg.info("답변 생성 중...")
...
for msg_idx, message in enumerate(st.session_state.messages):
    with st.chat_message(message["role"]):
        ...
        if message["role"] == "assistant" and message.get("citations"):
            ...
            if show_expert_mode:
                with st.expander("추론 과정 분석"):
                    df_data = []
                    for i, d in enumerate(citations):
                        df_data.append({
                            "최종 순위": i + 1,
                            "출처": d.metadata.get(MetadataFields.SRC_NAME) or "알 수 없음",
                            "페이지": d.metadata.get(MetadataFields.PG_NUM) or "-",
                            "벡터 점수": d.metadata.get("score", 0.0),
                            "리랭킹 점수": d.metadata.get("rerank_score", 0.0),
                        })
            ...
            for i, doc in enumerate(citations):
                source = doc.metadata.get(MetadataFields.SRC_NAME) or "알 수 없음"
...
if prompt := st.chat_input("규정에 대해 궁금한 점을 물어보세요."):
    ...
    with st.chat_message("assistant"):
        ...
        status = st.status("파이프라인 진행 중...", expanded=True)
        ...
        try:
            ...
            st.session_state.messages.append({"role": "assistant", "content": ui_handler.full_response, "citations": ui_handler.final_docs})
            perf_logger.log("Total", time.time() - start_time, prompt[:30])
            st.rerun()
        except Exception as e:
            logger.error(f"오류: {e}", exc_info=True)
            status.update(label="실행 실패", state="error", expanded=True)
            st.session_state.messages.append({"role": "assistant", "content": ui_handler.handle_error(e), "citations": ui_handler.final_docs})
