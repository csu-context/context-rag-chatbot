...
class RagasEvaluator:
    ...
    def __init__(
        self, eval_model_type: str = "claude", eval_model_name: str = "claude-haiku-4-5-20251001"
    ):
        # 평가를 위한 LLM 설정 (가성비 좋은 Haiku 4.5 사용)
        self.eval_llm_inst = LLMFactory.create_llm(model_type=eval_model_type, model_name=eval_model_name, temperature=0)
        self.eval_llm = self.eval_llm_inst.get_model()
        self.eval_model_name = eval_model_name

        # RAGAS는 임베딩 모델도 필요함 (Gemini 임베딩 기본값 유지)
        self.api_key = os.getenv("GOOGLE_API_KEY")
        self.eval_embeddings = GoogleGenerativeAIEmbeddings(model="models/gemini-embedding-001", google_api_key=self.api_key)

        # RAG 파이프라인 초기화
        self.db_manager = ChromaDBManager()
        # 토큰 추적을 위한 별도의 LLM 인스턴스 (Sonnet)
        self.rag_llm_inst = LLMFactory.create_llm()
    ...
    def _calculate_cost(self, usage_dict):
        ...
        cost = (usage_dict["input"] * rates["input"] / 1000000) + (usage_dict["output"] * rates["output"] / 1000000)
        return cost
    ...
    def prepare_dataset(self, golden_dataset_path: Path, max_samples: int = 20, delay: float = 1.0) -> Dataset:
        ...
        for i, item in enumerate(golden_data):
            ...
            try:
                ...
                context_str = "\n\n".join([f"내용: {res['content']}\n출처: [{res['metadata'].get('src_name')}, p.{res['metadata'].get('pg_num')}]" for res in search_results])
        ...
        return Dataset.from_dict({"question": questions, "answer": answers, "contexts": contexts, "ground_truth": ground_truths})
    ...
    def run_evaluation(self, dataset: Dataset, max_workers: int = 2):
        ...
        try:
            ...
            result.scores["total_eval_time_sec"] = elapsed
            return result
        except Exception as e:
    ...
    def save_results(self, result, output_dir: Path):
        ...
        summary_data = {
            ...
            "estimated_cost_usd": {"inference": inf_cost}
        }
        ...
        print("\n" + "=" * 45)
        print("      최종 평가 및 비용 리포트")
        print("=" * 45)
        print(f"- 추론 모델: {self.total_usage['inference']['model']}")
        print(f"- 추론 비용: ${inf_cost:.4f} ({self.total_usage['inference']['input'] + self.total_usage['inference']['output']:,} 토큰)")
        print(f"- 평가 모델: {self.eval_model_name} (Haiku)")
        print(f"- 총 소요 시간: {result.scores.get('total_eval_time_sec', 0):.1f}초")
        print("-" * 45)
        print("지표 결과 (RAGAS Scores):")
        for m, s in result.scores.items():
            if isinstance(s, (int, float)) and m != "total_eval_time_sec":
                print(f"  * {m:18}: {s:.4f}")
        print("=" * 45)
