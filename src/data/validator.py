...
class ParsingValidator:
    ...
    def get_processed_files(self) -> list[Path]:
        """manifest.json을 제외한 모든 전처리 결과 파일 경로 반환"""
    ...
    def analyze(self):
        ...
        logger.info(f"대상 파일 수: {len(processed_files)}개")
        ...
        # 메타데이터 추출 (부모 청크 기준)
        sources = [d["metadata"][MetadataFields.SRC_NAME] for d in all_data]
        ...
        # 자식 청크 통계
        total_child_chunks = sum(len(d["children"]) for d in all_data)
        logger.info(f"총 생성된 자식 청크 수: {total_child_chunks}")

        # 탐지 실패율 계산 (기본값 포함 여부 확인)
        no_chapter = [d for d in all_data if "장 없음" in d["metadata"].get("chapter", "")]
        no_article = [d for d in all_data if "조 없음" in d["metadata"].get("article", "")]
        ...
        # 본문 길이 분석
        child_lengths = [len(c["text"]) for d in all_data for c in d["children"]]
        if child_lengths:
        ...
        if all_data:
            ...
            if sample_child:
                logger.info(f"   [Chunk ID]  : {sample_child['metadata'][MetadataFields.CHUNK_ID]}")
