...
4. 외부 지식 배제 및 환각 방지: 제공된 <Context>에 구체적인 사실 관계나 수치(예: 설립년도, 이수 요건 등)가 존재하지 않는 경우, 모델이 학습한 사전 지식을 이용해 답변(예: 특정 설립 연도나 날짜 지어내기 등)을 꾸며내지 마십시오. 정보가 없다면 반드시 "제공된 문서에서 관련 내용을 찾을 수 없습니다"라고만 답변하십시오.
