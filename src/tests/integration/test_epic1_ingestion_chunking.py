...
def test_korean_chunker_separators():
    chunker = HierarchicalChunker(
        parent_chunk_size=100, parent_chunk_overlap=10, child_chunk_size=30, child_chunk_overlap=5
    )
...
def test_parent_document_mapping_in_pipeline(tmp_path, monkeypatch):
    ...
    mock_processed_data = [
        {
            ...
            "metadata": {"source_id": source_id, "parent_id": None, "chunk_id": parent_id},
            "children": [
                {
                    "chunk_id": child_id,
                    "metadata": {"source_id": source_id, "parent_id": parent_id, "chunk_id": child_id},
                    "text": "자식 청크 본문",
                }
            ],
        }
    ...
    # 입력 자식 문서 객체 구성
    child_doc = Document(
        page_content="자식 청크 본문", metadata={"source_id": source_id, "parent_id": parent_id, "chunk_id": child_id}
    )
